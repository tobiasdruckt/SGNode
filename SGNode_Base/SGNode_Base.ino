...
Theme lightTheme = {
  .background = rgb565(244, 246, 249),      // Soft cool paper
  .cardBackground = rgb565(238, 241, 246),  // Quiet card surface
  .buttonInactive = rgb565(225, 230, 238),  // Gentle inactive controls
  .primary = rgb565(233, 238, 245),         // Soft top bar
  .primaryText = rgb565(26, 40, 56),        // Soft navy text
  .accent = rgb565(226, 232, 241),          // Subtle accent surface
  .accentText = rgb565(255, 255, 255),     // White
  .gold = rgb565(185, 142, 58),            // Softer amber accent
  .textPrimary = rgb565(26, 34, 46),       // Deep slate (less harsh than black)
  .textSecondary = rgb565(64, 76, 92),     // Secondary slate
  .textMuted = rgb565(108, 120, 136),      // Muted slate
  .success = rgb565(74, 154, 94),          // Softer green
  .warning = rgb565(206, 141, 43),         // Softer amber
  .error = rgb565(203, 89, 89),            // Softer red
  .info = rgb565(73, 132, 196),            // Softer blue
  .border = rgb565(206, 214, 224),         // Soft border
  .gridLine = rgb565(219, 226, 235),       // Soft grid
  .graphPurple = rgb565(142, 100, 174),    // Muted purple for ABV
  .graphBlue = rgb565(73, 132, 196),       // Muted blue for SG
  .graphGreen = rgb565(74, 154, 94)        // Muted green for Temperature
};
...
bool sdWriteErrorActive = false;
unsigned long lastSDWriteFailure = 0;
unsigned long oldestBufferedMillis = 0;
...
int graphXForHour(float hour, int graphX, int graphW, float chartHours);
float pointHourForBatchChart(int pointIndex);
void drawTargetEventMarker(int gx, int gy, int gw, int gh, float chartHours,
...
void rollbackBatchFiles(const char* batchId);
...
bool shouldShowSdWarning();
void loadAllHistoryForDetailChart();
...
void loop() {
  ...
  if (screenDirty) {
    ...
    if (topbarDirty) {
      ...
      if (getLatestDisplayBatteryVoltage(&latestVoltage)) {
        battPercent = calculateBatteryPercentage(latestVoltage);
      }
...
void initRTC() {
  ...
  if (!isEpochValid(rtcEpoch) || rtcEpoch < buildEpoch) {
    ...
    if (setRTCDateTime(year, month, day, hour, minute, second)) {
      syncSystemTimeFromRTC(buildEpoch);
      char ts[24];
    ...
    syncSystemTimeFromRTC(rtcEpoch);
    char ts[24];
...
void onDataReceived(const uint8_t *mac, const uint8_t *incomingData, int len) {
  if (len == sizeof(payload_t)) {
    ...
    payload_t normalizedData = receivedData;
    bool batteryPlausible = !isnan(receivedData.battery_voltage) &&
                            !isinf(receivedData.battery_voltage) &&
                            receivedData.battery_voltage >= 2.5f &&
                            receivedData.battery_voltage <= 4.4f;
    if (!batteryPlausible) {
      float fallbackVoltage = 0.0f;
      if (getLatestDisplayBatteryVoltage(&fallbackVoltage)) {
        normalizedData.battery_voltage = fallbackVoltage;
      } else {
        normalizedData.battery_voltage = 0.0f;
      }
      LOG_ERROR("Invalid battery sample %.2fV sanitized to %.2fV\n",
    ...
    LOG_VERBOSE("Received: seq=%d, angle=%.2fÂ°, density=%.3f, temp=%.1fÂ°C, battery=%.2fV (%d%%) [%s]\n", 
                  normalizedData.sequence_id, normalizedData.angle, normalizedData.density,
                  normalizedData.temperature, normalizedData.battery_voltage, batteryPercent,
...
void drawDashboardScreen() {
  ...
  const int actionH = 86;
  const int metricH = 52;
  const int contextH = 38;
  char text[64];
  char value[24];

  // Summary strip: batch + phase only
  uiCard(MARGIN, contentY, contentW, summaryH, CARD_RADIUS);
  uiEllipsize(activeBrewProfile.batchName, contentW - 22, text, sizeof(text));
  tft.setTextColor(uiColorTextPrimary);
  ...
  // Primary focus: next action panel
  int actionY = contentY + summaryH + dashGap;
  ...
  tft.setTextColor(currentBatchAction.requiresChoice ? uiColorWarning : uiColorTextMuted);
  tft.setFreeFont(FONT_SIZE_XS);
  tft.setCursor(MARGIN + 10, actionY + 13);
  ...
  int metricY = actionY + actionH + dashGap;
  int tileW = (contentW - GAP * 3) / 4;
  const char* labels[4] = {"Atten", "ABV", "FG", "ETA"};
  for (int i = 0; i < 4; i++) {
    int x = MARGIN + i * (tileW + GAP);
    uiCard(x, metricY, tileW, metricH, CARD_RADIUS);
    tft.setTextColor(uiColorTextMuted);
    tft.setFreeFont(FONT_SIZE_XS);
    tft.setCursor(x + 10, metricY + 14);
    tft.print(labels[i]);
    tft.setTextColor(uiColorTextPrimary);
    tft.setFreeFont(FONT_SIZE_SM);
    if (i == 0) {
      if (hasData) snprintf(value, sizeof(value), "%.0f%%", fermentationMetrics.currentAttenuation);
      else strcpy(value, "--");
    } else if (i == 1) {
      if (hasData) snprintf(value, sizeof(value), "%.1f%%", fermentationMetrics.estimatedABV);
      else strcpy(value, "--");
    } else if (i == 2) {
      snprintf(value, sizeof(value), "%.3f", activeBrewProfile.expectedFinalGravity);
    } else {
      if (currentETA.valid) formatDurationShort(currentETA.secondsToPackaging, value, sizeof(value));
      else strcpy(value, "--");
    }
    tft.setCursor(x + 10, metricY + 36);
    tft.print(value);
  }

  // Minimal context row (only what helps decisions)
  int contextY = metricY + metricH + dashGap;
  int halfW = (contentW - GAP) / 2;
  uiCard(MARGIN, contextY, halfW, contextH, CARD_RADIUS);
  tft.setTextColor(uiColorTextMuted);
  tft.setFreeFont(FONT_SIZE_XS);
  tft.setCursor(MARGIN + 10, contextY + 14);
  tft.print("OG");
  if (activeBrewProfile.ogNeedsChoice) strcpy(text, "Verify pending");
  else if (activeBrewProfile.ogVerified) snprintf(text, sizeof(text), "OK %.3f", activeBrewProfile.effectiveOG);
  else strcpy(text, "Measuring");
  tft.setTextColor(uiColorTextPrimary);
  tft.setCursor(MARGIN + 10, contextY + 30);
  tft.print(text);

  int tempX = MARGIN + halfW + GAP;
  uiCard(tempX, contextY, halfW, contextH, CARD_RADIUS);
  tft.setTextColor(uiColorTextMuted);
  tft.setFreeFont(FONT_SIZE_XS);
...
float targetHourForAttenuation(float attenuationPercent, float chartHours) {
  float og = activeBrewProfile.effectiveOG;
  float fg = activeBrewProfile.expectedFinalGravity;
  if (og <= fg || attenuationPercent <= 0.0f) return -1.0f;
  float targetSG = og - (og - fg) * attenuationPercent / 100.0f;
  return targetHourForSG(targetSG, chartHours);
}

float eventHourFromEpoch(unsigned long eventEpoch) {
  if (eventEpoch == 0 || !isEpochValid(eventEpoch) || !isEpochValid(activeBrewProfile.createdAt) ||
      eventEpoch < activeBrewProfile.createdAt) {
    return -1.0f;
  }
  return (eventEpoch - activeBrewProfile.createdAt) / 3600.0f;
}

int graphXForHour(float hour, int graphX, int graphW, float chartHours) {
  if (chartHours <= 0.0f) return graphX;
...
float pointHourForBatchChart(int pointIndex) {
  if (pointIndex < 0 || pointIndex >= displayDataCount) return -1.0f;
  uint32_t pointEpoch = timestampForPoint(pointIndex);
  if (brewProfileLoaded && isEpochValid(activeBrewProfile.createdAt) &&
      isEpochValid(pointEpoch) && pointEpoch >= activeBrewProfile.createdAt) {
    return (pointEpoch - activeBrewProfile.createdAt) / 3600.0f;
  }
  if (timestampsAvailableForGraph()) {
    uint32_t firstEpoch = timestampForPoint(0);
    if (isEpochValid(pointEpoch) && pointEpoch >= firstEpoch) {
      return (pointEpoch - firstEpoch) / 3600.0f;
    }
  }
  if (displayDataCount >= 2) {
    int idx = displayBufferIndexForPoint(pointIndex);
    int firstIdx = displayBufferIndexForPoint(0);
    uint32_t pointUptime = displayDataBuffer[idx].uptime_s;
    uint32_t firstUptime = displayDataBuffer[firstIdx].uptime_s;
    if (pointUptime >= firstUptime) return (pointUptime - firstUptime) / 3600.0f;
  }
  return 0.0f;
}
...
void drawTargetEventMarker(int gx, int gy, int gw, int gh, float chartHours,
                           float hour, const char* label, uint16_t color, int labelSlot, int labelSide) {
  if (hour < 0.0f || chartHours <= 0.0f || hour > chartHours) return;
  int px = gx + (int)((hour / chartHours) * gw);
  if (px < gx) px = gx;
  if (px > gx + gw) px = gx + gw;

  for (int y = gy; y < gy + gh; y += 6) {
    tft.drawFastVLine(px, y, 3, color);
  }
  tft.fillTriangle(px - 3, gy + 1, px + 3, gy + 1, px, gy + 7, color);

  tft.setTextColor(color);
  tft.setFreeFont(FONT_SIZE_XS);
  if (!label || label[0] == '\0') return;
  int labelY = gy + 24 + (labelSlot % 4) * 15;
  int labelW = tft.textWidth(label);
  int labelX = px - labelW / 2;
  if (labelSide < 0) labelX = px - labelW - 5;
  if (labelSide > 0) labelX = px + 5;
  if (labelX < gx + 2) labelX = gx + 2;
  if (labelX + labelW > gx + gw - 2) labelX = gx + gw - labelW - 2;
...
void drawTargetChartSpanLabel(int gx, int gy, int gw, float chartHours,
                              float startHour, float endHour, const char* label, uint16_t color, int row) {
  if (!label || label[0] == '\0' || chartHours <= 0.0f || endHour <= 0.0f || startHour >= chartHours) return;
  if (startHour < 0.0f) startHour = 0.0f;
  if (endHour > chartHours) endHour = chartHours;
  if (endHour <= startHour) return;

  int x0 = graphXForHour(startHour, gx, gw, chartHours);
  int x1 = graphXForHour(endHour, gx, gw, chartHours);
  int cx = (x0 + x1) / 2;
  tft.setTextColor(color);
  tft.setFreeFont(FONT_SIZE_XS);
  int labelW = tft.textWidth(label);
  ...
  if (labelX < gx + 2) labelX = gx + 2;
  if (labelX + labelW > gx + gw - 2) labelX = gx + gw - labelW - 2;
  tft.setCursor(labelX, gy + 18 + row * 14);
  tft.print(label);
}
...
void drawTargetChartXAxis(int gx, int gy, int gw, int gh, float chartHours) {
  ...
  for (int i = 0; i <= ticks; i++) {
    float hour = chartHours * (float)i / (float)ticks;
    int px = gx + (int)((hour / chartHours) * gw);
...
void drawTargetChartEventMarkers(int gx, int gy, int gw, int gh, float chartHours) {
  ...
  if (activeBrewProfile.diacetylRestEnabled || activeBrewProfile.diacetylRestRecommendedByYeast ||
    ...
    dRestHour = eventHourFromEpoch(activeBrewProfile.dRestStartedAt);
    if (dRestHour < 0.0f && !activeBrewProfile.dRestSkipped) {
      bool lagerLike = strcmp(activeBrewProfile.curveTemplate, "lager") == 0 ||
                       strstr(activeBrewProfile.yeastCategory, "Lager") != NULL;
      dRestHour = targetHourForAttenuation(lagerLike ? 72.0f : 78.0f, chartHours);
    }
  }

  if (activeBrewProfile.dryHopEnabled || activeBrewProfile.dryHopDone || activeBrewProfile.dryHopSkipped) {
    dryHopHour = eventHourFromEpoch(activeBrewProfile.dryHopStartTime);
    if (dryHopHour < 0.0f && !activeBrewProfile.dryHopSkipped) {
      ...
      dryHopHour = targetHourForSG(triggerSG, chartHours);
    }

    removeHour = eventHourFromEpoch(activeBrewProfile.dryHopRemovedAt);
    if (removeHour < 0.0f && !activeBrewProfile.dryHopRemoveSkipped && dryHopHour >= 0.0f) {
      unsigned long contactHours = activeBrewProfile.dryHopContactHours > 0 ? activeBrewProfile.dryHopContactHours : 48UL;
      removeHour = dryHopHour + (float)contactHours;
    }
  }

  if (!activeBrewProfile.coldCrashSkipped || activeBrewProfile.coldCrashDone) {
  ...
  if (coldHour >= 0.0f && coldHour < yeastEnd) yeastEnd = coldHour;
  drawTargetChartSpanLabel(gx, gy, gw, chartHours, 0.0f, yeastEnd, "Yeast range", currentTheme->graphPurple, 0);

  if (dRestHour >= 0.0f) {
    float dRestEnd = chartHours;
    if (dryHopHour > dRestHour && dryHopHour < dRestEnd) dRestEnd = dryHopHour;
    if (coldHour > dRestHour && coldHour < dRestEnd) dRestEnd = coldHour;
    drawTargetEventMarker(gx, gy, gw, gh, chartHours, dRestHour, "", uiColorError, slot++, 0);
    drawTargetChartSpanLabel(gx, gy, gw, chartHours, dRestHour, dRestEnd, "D-Rest", uiColorError, 0);
  }

  if (dryHopHour >= 0.0f) {
    drawTargetEventMarker(gx, gy, gw, gh, chartHours, dryHopHour, "", currentTheme->graphGreen, slot++, 0);
    if (removeHour >= 0.0f) {
      drawTargetEventMarker(gx, gy, gw, gh, chartHours, removeHour, "", currentTheme->graphGreen, slot++, -1);
      drawTargetChartSpanLabel(gx, gy, gw, chartHours, dryHopHour, removeHour, "Cold Hop", currentTheme->graphGreen, 0);
    }
  }
...
void drawTargetVsActualChart() {
  ...
  drawTargetChartEventMarkers(gx, gy, gw, gh, chartHours);
  ...
  for (int i = 0; i <= 60; i++) {
    ...
    else if (progress < 0.65f) band = 0.004f;
    else if (progress < 0.90f) band = 0.003f;
    int px = gx + (i * gw / 60);
    int py = gy + gh - (int)((sg - minSG) * gh / (maxSG - minSG));
    int upperY = gy + gh - (int)(((sg + band) - minSG) * gh / (maxSG - minSG));
    int lowerY = gy + gh - (int)(((sg - band) - minSG) * gh / (maxSG - minSG));
    if (upperY < gy) upperY = gy;
    if (upperY > gy + gh) upperY = gy + gh;
    if (lowerY < gy) lowerY = gy;
    if (lowerY > gy + gh) lowerY = gy + gh;
    if (prevUpperX >= 0) {
      if (i % 2 == 0) {
        tft.drawLine(prevUpperX, prevUpperY, px, upperY, uiColorTextMuted);
        tft.drawLine(prevLowerX, prevLowerY, px, lowerY, uiColorTextMuted);
      }
    }
...
void drawGraphForMetric(GraphMetric metric) {
  ...
  for (int i = 0; i < displayDataCount; i++) {
    ...
    if (metric != METRIC_TEMPERATURE && metric != METRIC_ANGLE) {
  ...
  if (metric == METRIC_ABV && brewProfileLoaded) {
    ...
    maxValue += 0.5f;
    if (maxValue < 2.0f) maxValue = 2.0f;
    if (maxValue > 8.0f) maxValue = 8.0f;
  } else if (metric == METRIC_ANGLE) {
    float angleLowSG = angleForTargetSG(0.950f);
    float angleHighSG = angleForTargetSG(1.150f);
    minValue = floorf(min(angleLowSG, angleHighSG));
    maxValue = ceilf(max(angleLowSG, angleHighSG));
    if (maxValue - minValue < 8.0f) {
      float mid = (minValue + maxValue) * 0.5f;
      minValue = floorf(mid - 4.0f);
      maxValue = ceilf(mid + 4.0f);
    }
    if (minValue < 0.0f) minValue = 0.0f;
    if (maxValue > 90.0f) maxValue = 90.0f;
    if (maxValue <= minValue) maxValue = minValue + 10.0f;
  }
  
  // Add padding
  minValue -= padding;
  maxValue += padding;
  if (metric == METRIC_ABV && maxValue < 2.0f) maxValue = 2.0f;
  if (metric == METRIC_ABV && maxValue > 8.0f) maxValue = 8.0f;
  ...
  tft.setTextColor(uiColorTextSecondary);
  tft.setFreeFont(FONT_SIZE_XS);
  tft.setCursor(graphX - 42, graphY + 10);
  tft.print(yAxisLabel);
  
  // Update Y-axis min/max values
  char buf[16];
  snprintf(buf, sizeof(buf), metric == METRIC_DENSITY ? "%.3f" : (metric == METRIC_ANGLE ? "%.0f" : "%.1f"), maxValue);
  tft.setCursor(graphX - 35, graphY + 5);
  tft.print(buf);
  
  snprintf(buf, sizeof(buf), metric == METRIC_DENSITY ? "%.3f" : (metric == METRIC_ANGLE ? "%.0f" : "%.1f"), minValue);
  ...
  if (useTargetTimebase) {
    drawTargetChartXAxis(graphX, graphY, graphW, graphH, chartHours);
  } else if (displayDataCount >= 2) {
    payload_t firstData = displayDataBuffer[(displayDataIndex - displayDataCount + MAX_DATA_POINTS) % MAX_DATA_POINTS];
  ...
  if (metric == METRIC_TEMPERATURE && brewProfileLoaded) {
    ...
    float dRestHigh = yeastHigh + 3.0f;
    if (dRestHigh > 30.0f) dRestHigh = 30.0f;
    if (dRestLow > dRestHigh - 1.0f) dRestLow = dRestHigh - 1.0f;
    float coldLow = 1.0f;
    float coldHigh = 4.0f;
    float dRestHour = -1.0f;
    if (activeBrewProfile.diacetylRestEnabled || activeBrewProfile.diacetylRestRecommendedByYeast ||
    ...
    float dryHopHour = -1.0f;
    float hopOutHour = -1.0f;
    if ((activeBrewProfile.dryHopEnabled || activeBrewProfile.dryHopDone || activeBrewProfile.dryHopSkipped) &&
        !activeBrewProfile.dryHopRemoveSkipped) {
      dryHopHour = eventHourFromEpoch(activeBrewProfile.dryHopStartTime);
      if (dryHopHour < 0.0f && !activeBrewProfile.dryHopSkipped) {
        float triggerSG = activeBrewProfile.dryHopTriggerSG > 0.0f ? activeBrewProfile.dryHopTriggerSG : 1.014f;
        float fgBasedTrigger = activeBrewProfile.expectedFinalGravity > 1.0f
      ...
      hopOutHour = eventHourFromEpoch(activeBrewProfile.dryHopRemovedAt);
      if (hopOutHour < 0.0f && dryHopHour >= 0.0f) {
        unsigned long contactHours = activeBrewProfile.dryHopContactHours > 0 ? activeBrewProfile.dryHopContactHours : 48UL;
        hopOutHour = dryHopHour + (float)contactHours;
      }
      if (hopOutHour >= 0.0f && coldHour < hopOutHour + 2.0f) {
        coldHour = hopOutHour + 2.0f;
      }
    }

    float phaseHours[120];
    int phaseHourCount = 0;
    for (int i = 0; i <= 80 && phaseHourCount < 120; i++) {
      phaseHours[phaseHourCount++] = chartHours * (float)i / 80.0f;
    }
    if (dRestHour >= 0.0f && phaseHourCount + 2 < 120) {
      phaseHours[phaseHourCount++] = dRestHour;
      phaseHours[phaseHourCount++] = dRestHour + 12.0f;
    }
    if (dryHopHour >= 0.0f && phaseHourCount < 120) phaseHours[phaseHourCount++] = dryHopHour;
    if (hopOutHour >= 0.0f && phaseHourCount < 120) phaseHours[phaseHourCount++] = hopOutHour;
    if (coldHour >= 0.0f && phaseHourCount + 2 < 120) {
      phaseHours[phaseHourCount++] = coldHour;
      phaseHours[phaseHourCount++] = coldHour + 24.0f;
    }
    for (int i = 0; i < phaseHourCount - 1; i++) {
      for (int j = i + 1; j < phaseHourCount; j++) {
        if (phaseHours[j] < phaseHours[i]) {
          float tmp = phaseHours[i];
          phaseHours[i] = phaseHours[j];
          phaseHours[j] = tmp;
        }
      }
    }

    int prevX = -1, prevLowY = -1, prevHighY = -1;
    for (int i = 0; i < phaseHourCount; i++) {
      float hour = phaseHours[i];
      if (hour < 0.0f) hour = 0.0f;
      if (hour > chartHours) hour = chartHours;
      float low = yeastLow;
      float high = yeastHigh;
      uint16_t color = currentTheme->graphPurple;
      if (dRestHour >= 0.0f && hour >= dRestHour) {
        float t = (hour - dRestHour) / 12.0f;
        if (t > 1.0f) t = 1.0f;
        low = yeastLow + (dRestLow - yeastLow) * t;
        high = yeastHigh + (dRestHigh - yeastHigh) * t;
        color = uiColorError;
      }
      if (dryHopHour >= 0.0f && hour >= dryHopHour &&
          (hopOutHour < 0.0f || hour < hopOutHour)) {
        color = currentTheme->graphGreen;
      }
      if (coldHour >= 0.0f && hour >= coldHour) {
        float startLow = (dRestHour >= 0.0f && coldHour >= dRestHour) ? dRestLow : yeastLow;
        float startHigh = (dRestHour >= 0.0f && coldHour >= dRestHour) ? dRestHigh : yeastHigh;
        float t = (hour - coldHour) / 24.0f;
        if (t > 1.0f) t = 1.0f;
        low = startLow + (coldLow - startLow) * t;
        high = startHigh + (coldHigh - startHigh) * t;
        color = currentTheme->graphBlue;
      }
      int x = graphXForHour(hour, graphX, graphW, chartHours);
      int lowY = graphY + graphH - ((low - minValue) / valueRange * graphH);
      int highY = graphY + graphH - ((high - minValue) / valueRange * graphH);
      if (lowY < graphY) lowY = graphY;
      if (lowY > graphY + graphH) lowY = graphY + graphH;
      if (highY < graphY) highY = graphY;
      if (highY > graphY + graphH) highY = graphY + graphH;
      if (prevX >= 0) {
        drawGraphLine(prevX, prevLowY, x, lowY, color);
        drawGraphLine(prevX, prevHighY, x, highY, color);
      }
      prevX = x;
      prevLowY = lowY;
      prevHighY = highY;
    ...
    int yYeastHigh = graphY + graphH - ((yeastHigh - minValue) / valueRange * graphH);
    int yYeastLow = graphY + graphH - ((yeastLow - minValue) / valueRange * graphH);
    snprintf(buf, sizeof(buf), "%.0f", yeastHigh);
    tft.setCursor(graphX - 35, yYeastHigh + 4);
    tft.print(buf);
    snprintf(buf, sizeof(buf), "%.0f", yeastLow);
    tft.setCursor(graphX - 35, yYeastLow + 4);
    tft.print(buf);
    if (dRestHour >= 0.0f) {
      int xRest = graphXForHour(dRestHour, graphX, graphW, chartHours);
      int yRest = graphY + graphH - ((dRestHigh - minValue) / valueRange * graphH);
      tft.setTextColor(uiColorError);
      snprintf(buf, sizeof(buf), "%.0f", dRestHigh);
      tft.setCursor(graphX - 35, yRest + 4);
      tft.print(buf);
    }
    if (coldHour >= 0.0f) {
      int xCold = graphXForHour(coldHour, graphX, graphW, chartHours);
      int yCold = graphY + graphH - ((coldHigh - minValue) / valueRange * graphH);
      tft.setTextColor(currentTheme->graphBlue);
      snprintf(buf, sizeof(buf), "%.0f", coldHigh);
  ...
  for (int i = 0; i < displayDataCount; i++) {
    ...
    int x = graphXForPoint(i, graphX, graphW);
    if (useTargetTimebase) {
      float hour = pointHourForBatchChart(i);
      if (hour < 0.0f || hour > chartHours) continue;
      x = graphXForHour(hour, graphX, graphW, chartHours);
    }
...
void drawBatteryView() {
  ...
  int sparkY = graphY + 18;
  int sparkW = graphW - 70;  // Reduced width for y-axis labels
  int batteryButtonH = 34;
  int batteryButtonY = graphY + graphH - batteryButtonH - 12;
  int sparkH = batteryButtonY - sparkY - 16;
  ...
  // 6h button
  drawButton(buttonX, batteryButtonY, buttonW, batteryButtonH, "6h", batteryLoadMode == LOAD_6H);
  buttonX += buttonW + GAP;
  
  // 24h button
  drawButton(buttonX, batteryButtonY, buttonW, batteryButtonH, "24h", batteryLoadMode == LOAD_24H);
  buttonX += buttonW + GAP;
  
  // All button
  drawButton(buttonX, batteryButtonY, buttonW, batteryButtonH, "All", batteryLoadMode == LOAD_ALL);
  ...
  tft.setTextColor(uiColorTextSecondary);
  tft.setFreeFont(FONT_SIZE_SM);
  tft.setCursor(cardX + 15, graphY + 18);
  tft.print("Voltage");
  
  float latestBatteryVoltage = 0.0f;
  bool hasValidBatteryVoltage = getLatestDisplayBatteryVoltage(&latestBatteryVoltage);
  if (hasValidBatteryVoltage) {
    tft.setTextColor(uiColorTextPrimary);
    tft.setFreeFont(FONT_SIZE_MD);
    char voltageBuf[16];
    snprintf(voltageBuf, sizeof(voltageBuf), "%.2fV", latestBatteryVoltage);
    tft.setCursor(cardX + 15, graphY + cardH - 12);
    tft.print(voltageBuf);
  }
  ...
  tft.setTextColor(uiColorTextSecondary);
  tft.setFreeFont(FONT_SIZE_SM);
  tft.setCursor(cardX + 15, graphY + cardH + GAP + 18);
  tft.print("Percentage");
  
  if (hasValidBatteryVoltage) {
    uint8_t percent = calculateBatteryPercentage(latestBatteryVoltage);
    tft.setTextColor(uiColorTextPrimary);
    tft.setFreeFont(FONT_SIZE_MD);
    char percentBuf[16];
    snprintf(percentBuf, sizeof(percentBuf), "%d%%", percent);
    tft.setCursor(cardX + 15, graphY + cardH + GAP + cardH - 12);
  ...
  tft.setTextColor(uiColorTextSecondary);
  tft.setFreeFont(FONT_SIZE_SM);
  tft.setCursor(cardX + 15, graphY + (cardH + GAP) * 2 + 18);
  tft.print("Status");
  ...
  if (chargingState == CHARGING) {
    ...
    tft.setFreeFont(FONT_SIZE_MD);
    tft.setCursor(cardX + 15, graphY + (cardH + GAP) * 2 + cardH - 12);
    tft.print("Charging");
...
void checkTouch() {
  ...
  if (touched) {
    ...
    if (currentMode == BATTERY_VIEW) {
      ...
      int buttonX = graphX + MARGIN;
      int buttonH = 34;
      int buttonY = graphY + graphH - buttonH - 12;
  ...
  if (calibResp.response_type == 0) {
    ...
    // Handle exit calibration response (request_id=254) - float unit exiting calibration
      ...
      if (calibResp.sg >= 0.995 && calibResp.sg <= 1.005) {
        ...
        LOG_INFO("Stored Point 1 angle: %.2fÂ°\n", calibAngles[0]);
...
bool loadHistoricalDataFromCSV(const char* filename, 
  ...
  payload_t recentDischargeData[25];
  uint32_t recentDischargeEpochs[25];
  int recentDischargeCount = 0;
  ...
  int eligibleDataLines = 0;
  file.seek(0);
  int eligibleLineNum = 0;
  while (file.available()) {
    int bytesRead = file.readBytesUntil('\n', line, sizeof(line) - 1);
    line[bytesRead] = '\0';
    eligibleLineNum++;
    if (eligibleLineNum == 1 || line[0] == '#') {
      continue;
    }
    payload_t data = {0};
    uint32_t epoch_s = 0;
  ...
  actualSamplingStep = eligibleDataLines > maxPoints
    ? (eligibleDataLines + maxPoints - 1) / maxPoints
    : 1;
  LOG_INFO("CSV history: eligible=%d sampleStep=%d\n", eligibleDataLines, actualSamplingStep);
  file.seek(0);
  ...
  while (file.available() && loadedCount < linesToLoad) {
    ...
    if (parsed) {
      ...
      if (data.battery_voltage >= 2.5f && data.battery_voltage <= 4.5f) {
        if (recentDischargeCount < 25) {
          recentDischargeData[recentDischargeCount] = data;
          recentDischargeEpochs[recentDischargeCount] = epoch_s;
          recentDischargeCount++;
        } else {
          for (int i = 1; i < 25; i++) {
            recentDischargeData[i - 1] = recentDischargeData[i];
            recentDischargeEpochs[i - 1] = recentDischargeEpochs[i];
          }
          recentDischargeData[24] = data;
          recentDischargeEpochs[24] = epoch_s;
        }
      }

      // Apply sampling only to the visible chart buffer. The battery ETA uses
      // the unsampled recentDischargeData window above.
      ...
      if (loadedCount <= 3 || loadedCount > linesToLoad - 3) {
        LOG_VERBOSE("Loaded point %d: SG=%.4f, Temp=%.1fÂ°C, Batt=%.2fV (%d%%)\n",
  ...
  dischargeRateBufferCount = recentDischargeCount;
  for (int i = 0; i < recentDischargeCount; i++) {
    dischargeRateBuffer[i] = recentDischargeData[i];
    dischargeRateTimestampBuffer[i] = recentDischargeEpochs[i];
  }
...
bool createFreshBatchLogFile(const char* batchId) {
  char logPath[80];
  BrewProfileStore::logPath(batchId, logPath, sizeof(logPath));
  if (SD.exists(logPath) && !SD.remove(logPath)) {
    LOG_ERROR("Failed to remove existing batch log: %s\n", logPath);
    return false;
  }
  File file = SD.open(logPath, FILE_WRITE);
  if (!file) {
...
void completeBrewWizard() {
  ...
  if (mountSDTemporarily()) {
    rollbackBatchFiles(activeBrewProfile.batchId);
    bool dirOK = BrewProfileStore::ensureBatchDirectory(activeBrewProfile.batchId);
    bool profileOK = dirOK && BrewProfileStore::save(activeBrewProfile);
    bool targetOK = profileOK && TargetCurveGenerator::generateAndSave(activeBrewProfile);
    bool logOK = targetOK && createFreshBatchLogFile(activeBrewProfile.batchId);
    bool activeOK = logOK && BrewProfileStore::saveActiveBatchId(activeBrewProfile.batchId);
    if (activeOK) {
      ...
      LOG_ERROR("Batch commit failed dir=%d profile=%d target=%d log=%d active=%d\n",
                dirOK ? 1 : 0, profileOK ? 1 : 0, targetOK ? 1 : 0, logOK ? 1 : 0, activeOK ? 1 : 0);
      rollbackBatchFiles(activeBrewProfile.batchId);
    }
    dismountSD();
  }
...
bool logDataToSD(payload_t data, uint32_t epoch_s) {
  ...
  if (bytesWritten > 0) {
    LOG_VERBOSE("Logged data to SD: SG=%.4f, Temp=%.2fÂ°C, Battery=%.2fV (%d%%)\n", 
...
bool getLatestDisplayBatteryVoltage(float* voltageOut) {
  ...
  if (latestFloatDataValid &&
      latestFloatData.battery_voltage >= 2.5f &&
      latestFloatData.battery_voltage <= 4.4f) {
    *voltageOut = latestFloatData.battery_voltage;
    return true;
  }

  for (int i = 0; i < displayDataCount; i++) {
    int idx = (displayDataIndex - 1 - i + MAX_DATA_POINTS) % MAX_DATA_POINTS;
    float voltage = displayDataBuffer[idx].battery_voltage;
    if (voltage >= 2.5f && voltage <= 4.4f) {
      *voltageOut = voltage;
      return true;
    }
  }
...
bool shouldShowSdWarning() {
  ...
  if (sdWriteErrorActive) return true;
  if (bufferedCount >= MAX_DATA_BUFFER / 10) return true;
  if (oldestBufferedMillis > 0 && millis() - oldestBufferedMillis > 30UL * 60UL * 1000UL) return true;
  return false;
}
...
void drawSdWarningTopbar() {
  ...
  int y = TOPBAR_H / 2 - 8;
  uint16_t color = uiColorError;
  tft.drawRoundRect(x, y, 16, 16, 2, color);
  tft.drawFastHLine(x + 4, y + 3, 8, color);
  tft.drawLine(x - 2, y + 17, x + 18, y - 3, color);
  tft.drawLine(x - 1, y + 17, x + 19, y - 3, color);
}

void loadAllHistoryForDetailChart() {
  if (!fermentationFileOpen || strlen(currentFermentationFile) == 0) return;
  if (currentLoadMode == LOAD_ALL && lastHistoricalLoadedLines > 0) return;
  if (!mountSDTemporarily()) return;
...
float calculateDischargeRateLinearRegression() {
  ...
  // Calculate correlation coefficient (RÂ²) to assess regression quality
  float meanX = sumX / n;
  float meanY = sumY / n;
  ...
  // Only trust regression if correlation is reasonable (RÂ² > 0.01)
  if (rSquared < 0.01) {
    LOG_VERBOSELN("  Poor correlation (RÂ² < 0.01), returning 0.0");
...
void bufferSensorData(payload_t data, uint32_t epoch_s) {
  if (bufferedCount < MAX_DATA_BUFFER) {
    ...
    if (oldestBufferedMillis == 0) oldestBufferedMillis = dataBuffer[bufferIndex].buffered_millis;
    bufferIndex = (bufferIndex + 1) % MAX_DATA_BUFFER;
    bufferedCount++;
    
    if (bufferedCount % 50 == 0) { // Only log every 50th message to reduce spam
      LOG_VERBOSE("Buffered sensor data: seq=%d, sg=%.3f, temp=%.1fÂ°C (buffer: %d/%d)\n", 
    ...
    oldestBufferedMillis = currentTime;
...
for (int i = 0; i < bufferedCount; i++) {
  if (!dataBuffer[i].written && dataBuffer[i].buffered_millis > 0 &&
      (oldestBufferedMillis == 0 || dataBuffer[i].buffered_millis < oldestBufferedMillis)) {
    oldestBufferedMillis = dataBuffer[i].buffered_millis;
  }
}
if (writtenCount > 0 && bufferedCount == 0) {
  sdWriteErrorActive = false;
} else if (writtenCount == 0 && bufferedCount > 0) {
  sdWriteErrorActive = true;
  lastSDWriteFailure = millis();
}
topbarDirty = true;
...
void calculateGraphPoints() {
  // Generate 91 points (0Â° to 90Â°) using current calibration polynomial
  for (int i = 0; i <= 90; i++) {
...
#if SGNODE_BATTERY_DEBUG
static char batteryDebugCommand[96];
static size_t batteryDebugCommandLength = 0;

static void batteryDebugPrint(bool ok, const char* message) {
  Serial.print(ok ? "OK " : "ERR ");
  Serial.println(message);
}

static bool batteryDebugStats(char* buffer, size_t bufferSize) {
  if (displayDataCount < 2) {
    snprintf(buffer, bufferSize, "battery_stats points=%d insufficient csv=%d/%d",
             displayDataCount, lastHistoricalLoadedLines, totalCSVDataLines);
    return false;
...
static bool batteryDebugBatchDiagnostics(char* buffer, size_t bufferSize) {
  if (!brewProfileLoaded) {
    snprintf(buffer, bufferSize,
             "batch_diag loaded=0 live=%d csv=%d/%d skip=%d err=%d relaxed=%d pre=%d post=%d",
             displayDataCount, lastHistoricalLoadedLines, totalCSVDataLines,
             lastHistoricalSkippedLines, lastHistoricalParseErrors,
             lastHistoricalBatchStartRelaxed ? 1 : 0,
             lastHistoricalPreBatchLines, lastHistoricalPostBatchLines);
...
static bool batteryDebugFloatStats(char* buffer, size_t bufferSize) {
  if (displayDataCount <= 0 && !latestFloatDataValid) {
    snprintf(buffer, bufferSize, "float_stats no_data");
    return false;
  }
  ...
  snprintf(buffer, bufferSize,
           "float_stats seq=%u sg=%.4f temp=%.2f batt=%.3f pct=%u angle=%.2f epoch=%lu valid=%d",
           latest.sequence_id, latest.density, latest.temperature, latest.battery_voltage,
           calculateBatteryPercentage(latest.battery_voltage), latest.angle, (unsigned long)latestEpoch,
           latestFloatDataValid ? 1 : 0);
  return true;
}

static bool batteryDebugDumpCsv() {
  if (!currentFermentationFile[0]) {
    batteryDebugPrint(false, "battery_dump_csv no_active_file");
    return false;
  }
  if (!mountSDTemporarily()) {
    batteryDebugPrint(false, "battery_dump_csv sd_mount_failed");
    return false;
  }
  ...
  if (!file || file.isDirectory()) {
    if (file) file.close();
    dismountSD();
    batteryDebugPrint(false, "battery_dump_csv open_failed");
    return false;
  }

  Serial.print("OK battery_dump_csv begin file=\"");
  ...
  while (file.available()) {
    ...
    line[pos] = '\0';
    pos = 0;
    if (line[0] == '\0' || strncmp(line, "timestamp,", 10) == 0) {
      continue;
    }
    ...
    uint32_t epoch = 0;
    uint8_t batteryPercent = 0;
    if (!parseCSVDataLine(line, &data, &epoch, &batteryPercent) || !isEpochValid(epoch)) {
      skipped++;
      continue;
    }
    parsed++;
    ...
    Serial.print(',');
    Serial.print(data.battery_voltage, 3);
    Serial.print(',');
    Serial.print(data.temperature, 2);
    Serial.print(',');
    Serial.print(data.density, 4);
    Serial.print(',');
  ...
  if (pos > 0) {
    ...
    uint8_t batteryPercent = 0;
    if (parseCSVDataLine(line, &data, &epoch, &batteryPercent) && isEpochValid(epoch) &&
        data.battery_voltage >= 2.5f && data.battery_voltage <= 4.5f) {
      Serial.print("DATA ");
      Serial.print((unsigned long)epoch);
      Serial.print(',');
      Serial.print(data.battery_voltage, 3);
  ...
  char result[96];
  snprintf(result, sizeof(result), "battery_dump_csv end emitted=%d parsed=%d skipped=%d", emitted, parsed, skipped);
  batteryDebugPrint(true, result);
  return true;
}

static void batteryDebugHandleCommand(char* command) {
  char result[300];
  ...
  if (strcmp(command, "help") == 0) {
    ...
    batteryDebugPrint(batteryDebugStats(result, sizeof(result)), result);
  } else if (strcmp(command, "battery_all") == 0) {
    bool loaded = false;
    if (currentFermentationFile[0] && mountSDTemporarily()) {
      loaded = loadHistoricalDataFromCSV(currentFermentationFile, LOAD_ALL, MAX_DATA_POINTS, 1);
      dismountSD();
    }
    if (loaded) {
      batteryDebugStats(result, sizeof(result));
      batteryDebugPrint(true, result);
    } else {
      snprintf(result, sizeof(result), "battery_all load_failed file=\"%s\"", currentFermentationFile);
      batteryDebugPrint(false, result);
    }
  } else if (strcmp(command, "float_stats") == 0) {
    batteryDebugPrint(batteryDebugFloatStats(result, sizeof(result)), result);
  } else if (strcmp(command, "battery_dump_csv") == 0) {
    batteryDebugDumpCsv();
  } else {
    batteryDebugPrint(false, "unknown_command");
  }
}
...
void handleBatteryDebugSerial() {
  while (Serial.available() > 0) {
    ...
    if (c == '\r') continue;
    if (c == '\n') {
      batteryDebugCommand[batteryDebugCommandLength] = '\0';
      if (batteryDebugCommandLength > 0) {
        batteryDebugHandleCommand(batteryDebugCommand);
      }
      batteryDebugCommandLength = 0;
    } else if (batteryDebugCommandLength + 1 < sizeof(batteryDebugCommand)) {
      batteryDebugCommand[batteryDebugCommandLength++] = c;
    }
  }
}
#endif
